...
def read_json(root):
    ...
    with open(os.path.join(root, f'data.json'), 'r') as f:
        dataset = json.load(f)
    ...
    print('Procesing: testing number {}'.format(
        numTest))
...
def cases_list(dataset, path, folder, set, remake=False):
    ...
    for i in dataset[set]:
        image = os.path.isfile(os.path.join(path, i['image'].split('/')[0], i['image'].split('/')[1]))
        name = i['image']
        
        if image and not remake:
