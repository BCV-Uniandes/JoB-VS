...
def train_voxels_center(image, patch_size, label, foreground):
    """ Select the central voxels of the patches for testing """
    # Lower and upper bound to sample the central voxel (to avoid padding if
    # it is too close to the borders)
    im_shape, _ = image_shape(image)
    ...
    options = [
                [x // 2, y //2, z // 2], # center
                [high[0], high[1], high[2]], # center
                [low[0], low[1], low[2]], # center
                [x // 2, y //2, low[2]], # down
                [x // 2, y //2, z - low[2]], #Up
    ...
    if foreground:
        ...
        x1 = np.random.randint(low[0], high[0])
        y1 = np.random.randint(low[1], high[1])
        z1 = np.random.randint(low[2], high[2])
        r_coords = [x, y, z]
        coords = random.choice(options + [r_coords])
